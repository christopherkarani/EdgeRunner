...
public struct RMSNormKernel: Sendable {
    ...
    /// Execute RMS normalization with a pre-allocated weight buffer.
    /// Avoids re-creating the weight MTLBuffer on every call (56 calls per forward pass).
    ...
    /// Encode RMS normalization into an existing command buffer (no commit/await).
