...
struct Gemma4ModelConfigTests {
    ...
    @Test("Parses E4B hparams from public ModelConfig metadata")
    func parsesE4BHparamsFromModelConfigMetadata() throws {
        let metadata = Gemma4ModelConfigTests.makeReferenceModelConfigMetadata()
        let config = try Gemma4ModelConfig(modelConfigMetadata: metadata)
    ...
    @Test("Parses current public Gemma 4 GGUF metadata aliases")
    func parsesCurrentPublicGGUFMetadataAliases() throws {
        let metadata = Gemma4ModelConfigTests.makeCurrentPublicGGUFMetadata()
        let config = try Gemma4ModelConfig(metadata: metadata)
        ...
        #expect(config.perLayerDim == 256)
        #expect(config.perLayerVocabSize == 262144)
        #expect(config.ropeThetaLocal == 10_000.0)
        #expect(config.ropeThetaGlobal == 1_000_000.0)
        #expect(config.localRotaryDimension == 256)
        #expect(config.globalRotaryDimension == 512)
        #expect(config.partialRotaryFactor == 1.0)
    ...
    static func makeReferenceModelConfigMetadata() -> [String: MetadataValue] {
        var metadata: [String: MetadataValue] = [:]
        for (key, value) in makeReferenceMetadata() {
            if let converted = value.metadataValue {
    ...
    static func makeCurrentPublicGGUFMetadata() -> [String: GGUFMetadataValue] {
        [
            "general.architecture": .string("gemma4"),
            "gemma4.block_count": .uint32(42),
            "gemma4.embedding_length": .uint32(2560),
            ...
            "gemma4.attention.shared_kv_layers": .uint32(18),
            "gemma4.attention.sliding_window": .uint32(512),
            "gemma4.rope.freq_base": .float32(1_000_000.0),
            "gemma4.rope.freq_base_swa": .float32(10_000.0),
            ...
            "gemma4.attention.sliding_window_pattern": .array(
                (0..<42).map { index in
                    .bool(![5, 11, 17, 23, 29, 35, 41].contains(index))
                }
            ),
            "tokenizer.ggml.tokens": .array(
                (0..<262144).map { .string("token_\($0)") }
            ),
        ]
    }
}
